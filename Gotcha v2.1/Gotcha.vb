...
Module Gotcha
    ...
    Dim logFile As String = Application.StartupPath & "/logs/" & Date.Today.ToString("dd-MM-yyyy-H-mm-ss") & "_log"
    ...
    Private Function LogAsync(ByVal log As LogMessage) As Task
        If log.ToString.Contains("Ready") Then
            ...
            Colorize("|/__\|/__\|/__\|/__\|/__\|/__\|/_______\|/__\|/__\|/__\|/__\|")
            Colorize("")
            Colorize("[INFO]      - Basic Settings  -")
            Colorize("[INFO]      Channel: " & channel)
            Colorize("[INFO]      Token:" & token)
            Colorize("[INFO]      - Catch Settings -")
            Colorize("[INFO]      Whitelist Loaded...")
            If autoBal = True Then
                Colorize("[INFO]      AutoBal enabled...")
            Else
                Colorize("[INFO]      AutoBal disabled...")
            End If
            Colorize("[INFO]      - Spam Settings -")
            Colorize("[INFO]      Interval:" & spamInterval)
            If autoSpam = True Then
                Colorize("[INFO]      AutoSpam enabled...")

                MainTimer.Interval = spamInterval
                MainTimer.Start()
            Else
                Colorize("[INFO]      AutoSpam disabled...")
            End If
    ...
    Private Async Function MessageReceivedAsync(ByVal message As SocketMessage) As Task
        If message.Author.ToString.Contains("Pokécord") Then
            ...
            If message.Content.Contains("You caught a") Then
                ...
                If message.Content.Contains("Added to Pokédex") AndAlso autoBal = True Then
    ...
    Public Sub SpamChats()
        ...
        ' If _client.Guilds.First.Users("599819646526423040").Status = UserStatus.Online Then
    ...
    Private Sub tick2(ByVal sender As Object, ByVal e As Timers.ElapsedEventArgs)
        If GetAsyncKeyState(VK_F12) Then
            If botRunning = True Then
                MainTimer.Stop()
                botRunning = False
                Colorize("[INFO]      Bot paused...")
            Else
                MainTimer.Start()
                botRunning = True
                Colorize("[INFO]      Bot resumed...")
            End If
        End If
    End Sub
